            ...
            $customer_session_id = trim($order['cc_owner']);
                ...
                if ($order_query->RecordCount() > 0) {
                    ...
                    //if ($order['orders_status'] == MODULE_PAYMENT_PAGAMASTARDE_PREPARE_ORDER_STATUS_ID) {
                      ...
                      $db->Execute("update " . TABLE_ORDERS . " set orders_status = '" . (MODULE_PAYMENT_PAGAMASTARDE_ORDER_STATUS_ID > 0 ? (int)MODULE_PAYMENT_PAGAMASTARDE_ORDER_STATUS_ID : (int)DEFAULT_ORDERS_STATUS_ID) . "', last_modified = now() where orders_id = '" . (int)$order_id_from_pagantis . "'");
                    //}
